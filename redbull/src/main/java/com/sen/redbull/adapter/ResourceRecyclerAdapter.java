...
public class ResourceRecyclerAdapter extends RecyclerView.Adapter<ResourceRecyclerAdapter.ViewHolder> {
    private List<ResourSecondItemBean> mData;
    private Context mContext;
    int width;

    public ResourceRecyclerAdapter(Context context, List<ResourSecondItemBean> data) {
        mContext = context;
        mData = data;
    }
...
//    }
    ...
    public interface OnItemClickListener {
        void onItemClick(View view, int position, ResourSecondItemBean childItemBean);
    }
    ...
    public void onBindViewHolder(final ViewHolder holder, final int position) {
        ...
        ResourSecondItemBean itemBean = mData.get(position);
        String imgUrl = Constants.PATH_PICTURE + itemBean.getLessonimg();
        holder.img_course_item.setTag(imgUrl);
        holder.tv_course_name.setText( itemBean.getLe_name());
        holder.tv_create_time.setText("上传时间：" + itemBean.getDate());
